...
(function () {
  ...
  const State = {
    ...
    linksCache: new Map(),   // { animeId: { links:[] } } — liên kết live-fetch, cache theo phiên khi mở modal
    linksBusy: new Set(),    // id đang fetch liên kết — tránh request trùng khi modal render lại
    afLinks: null,           // liên kết đang soạn trong form admin (auto-fill AniList hoặc giữ từ record cũ)
    adminEmail: '',
  ...
  // Lưu file .ass vừa upload — CHỈ lưu tạm vào State, KHÔNG ghi cache ngay
  // Cache chỉ được ghi khi người dùng bấm ▶
...
function setupSubPopupEvents() {
  ...
  // Số cột hiện tại của lưới — đọc trực tiếp số track đã resolve của repeat(auto-fill, ...)
  // qua computed style → đúng ở MỌI độ rộng màn hình (kể cả vùng 1000–1200px mà hard-code
  ...
  // Chiều cao 1 hàng card (px): đo card trong DOM nếu có, không thì dùng lần đo trước,
  // chưa từng đo được thì ước lượng từ bề rộng card × tỷ lệ poster (3:4.2) + phần meta.
  function getAnimeRowH() {
    const grid = $('#animeGrid');
    if (!grid) return 0;
    ...
    if (card) {
      ...
      if (h > 0) { State.animeRowH = h + gap; return State.animeRowH; }
    }
    if (State.animeRowH > 0) return State.animeRowH;
    const cols = getAnimeCols();
    const gridW = grid.clientWidth || 0;
    if (cols > 0 && gridW > 0) {
      const colGap = parseFloat(getComputedStyle(grid).columnGap) || gap;
      const cardW = (gridW - (cols - 1) * colGap) / cols;
      return cardW * (4.2 / 3) + 90 + gap; // poster 3:4.2 + ~90px (padding + tiêu đề 2 dòng + meta)
    }
    return 0;
  }

  // Số hàng/trang: desktop màn ngang → đếm số hàng vừa chiều cao khả dụng của viewport
  // (header + filter bar nằm trên lưới, tính qua vị trí top của lưới theo document);
  // mobile/màn dọc → cố định 2 hàng như cũ.
  function getAnimeRowsPerPage() {
    const fallback = State.animeRowsPerPage || 2;
    if (!isDesktopLandscape()) { State.animeRowsUsed = fallback; return fallback; }
    const rowH = getAnimeRowH();
    if (rowH <= 0) { State.animeRowsUsed = fallback; return fallback; }
    const grid = $('#animeGrid');
  ...
  function syncAnimeRows(quiet) {
    ...
    const grid = $('#animeGrid');
    const card = grid && grid.querySelector('.anime-card');
    if (!card) return;
    const gap = parseFloat(getComputedStyle(grid).rowGap) || 14;
    const h = card.getBoundingClientRect().height;
    if (!(h > 0)) return;
  ...
  function renderAnimeDetail(a) {
    ...
    const seiyuuSection = seiyuu.length
      ? '<details class="detail-section detail-collapse">' +
          ...
          '<div class="seiyuu-grid">' +
            seiyuu.map((s) => {
              // 1 hàng 3 ô: ô 1 ảnh nhân vật tròn, ô 2 hai dòng tên (NV trên trái / DV dưới phải), ô 3 ảnh diễn viên tròn
    ...
    // Liên kết — đọc sẵn từ Supabase (admin auto-fill/backfill), fallback live-fetch khi record chưa có;
    // nằm ngay dưới mục Dàn diễn viên lồng tiếng; bấm tiêu đề sổ ra danh sách liên kết
    const linksSection =
  ...
  async function recordWatch(animeId) {
    if (!State.isAdmin) { toast('Bạn không có quyền.', 'error'); return false; }
  ...
  // Xoá một ngày đã xem khỏi danh sách (watch_dates) + giảm watch_count theo
  async function removeWatchDate(animeId, date) {
    if (!State.isAdmin) { toast('Bạn không có quyền.', 'error'); return false; }
    const a = State.animes.find((x) => String(x.id) === String(animeId));
    if (!a) return false;
...
// ── Fallback Jikan (MyAnimeList) khi AniList đang ngừng ───────────────────────
  ...
  // Danh sách tên để tìm (romaji, title, native, synonyms) — thử lần lượt tới khi có kết quả
  ...
  // Liên kết từ AniList (official site, trang chủ...) — GraphQL externalLinks + siteUrl
  ...
  function renderExtLinksInto(body, data) {
    ...
    const linkItem = (l) => {
      ...
      const badgeHtml = l.icon
        ? '<img class="ext-link-fav" src="' + esc(l.icon) + '" alt="" loading="lazy" onerror="this.remove()" />'
        : '<span class="ext-link-badge"' + (l.color ? ' style="background:' + esc(l.color) + '"' : '') + '>' + esc(linkInitials(l.site)) + '</span>';
      return (
        '<a class="ext-link" href="' + esc(l.url) + '" target="_blank" rel="noopener nofollow noreferrer" title="Mở: ' + esc(l.url) + '">' +
          badgeHtml +
          '<span class="ext-link-txt">' +
            '<span class="ext-link-title">' + esc(l.site) + '</span>' +
            '<span class="ext-link-domain">' + esc(host) + '</span>' +
          '</span>' +
          '<span class="ext-link-open">↗</span>' +
        '</a>'
      );
    };
  ...
  async function loadExternalLinks(a) {
    const body = $('#detailLinksBody');
    if (!body) return;
    ...
    try {
      // 1) AniList: tìm id theo tên, lấy externalLinks + siteUrl
      let result = null;
      let anilistId = null;
      for (const q of extLinkQueryCandidates(a)) {
        try { anilistId = await anilistFindIdByTitle(q); } catch (_e) { anilistId = null; }
        if (anilistId) break;
      }
      if (anilistId) {
        try { result = await anilistFetchExternalLinks(anilistId); } catch (_e) { result = null; }
      }
      // 2) Fallback Jikan/MAL (AniList lỗi hoặc liên kết rỗng)
      const needJikan = !result || !result.links.length;
      if (needJikan) {
        for (const q of extLinkQueryCandidates(a)) {
          try {
            result = mergeExtLinks(result, await jikanFetchExternalLinks(q));
            break;
  ...
  async function backfillExternalLinks() {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      try {
        let result = null;
        // 1) AniList: tìm id theo từng tên ứng viên
        let anilistId = null;
        for (const q of extLinkQueryCandidates(a)) {
          try { anilistId = await anilistFindIdByTitle(q); } catch (_e) { anilistId = null; }
          if (anilistId) break;
  ...
  function bindEvents() {
    ...
    function renderDropdown(q) {
      ...
      cacheDropdown.classList.remove('hidden', 'open-up');
      // Lật dropdown lên trên nếu phía dưới không đủ chỗ (cột phụ đề có cuộn riêng trên
      // desktop → dropdown mở xuống sát đáy cột dễ bị cắt mất)
      requestAnimationFrame(() => {
        if (cacheDropdown.classList.contains('hidden')) return;
        const wrap = cacheDropdown.parentElement;
        const scroller = cacheDropdown.closest('.playlist-column');
        if (!wrap || !scroller) return;
        const wr = wrap.getBoundingClientRect();
        const sr = scroller.getBoundingClientRect();
        const need = cacheDropdown.offsetHeight;
        if (sr.bottom - wr.bottom < need && wr.top - sr.top > need) {
          cacheDropdown.classList.add('open-up');
        }
      });
    }
    ...
    if (assCacheApplyBtn) {
      assCacheApplyBtn.addEventListener('click', async () => {
        ...
        const hasPendingAss = !!(State._pendingAssText);
        if (!ytRaw && !searchRaw && !hasPendingAss) { toast('Paste link YouTube hoặc chọn file sub trước!', 'error', 2000); return; }
        const idEl = $('#assCacheId');
        const titleEl = $('#assCacheTitle');
        ...
        if (ytRaw) {
          ...
          if (yid) {
            ...
            // Xóa tiêu đề cũ TRƯỚC KHI fetch để fallback hoạt động đúng
        ...
        // ── Load sub: ưu tiên file vừa upload (pending) → search dropdown → repo ──
        const curId = (idEl && idEl.value) ? idEl.value.trim() : '';
        const curTitle = (titleEl && titleEl.value) ? titleEl.value.trim() : '';

        // Trường hợp 1: có file .ass vừa upload (lưu tạm trong State)
        if (hasPendingAss && State._pendingAssText) {
          const subText = State._pendingAssText;
          const properName = buildAssCacheFileName(curId || 'unknown', curTitle || 'sub');
          // Ghi vào cache
          const pCache = readAssCache();
          pCache[properName] = { name: properName, text: subText, addedAt: Date.now(), videoId: curId, title: curTitle };
          writeAssCache(pCache);
          mergeAssCacheIntoSubs();
          renderAssCacheList();
          // Xoá file pending
          State._pendingAssText = null;
          State._pendingAssFileName = null;
          // Phát video
          const file = { name: properName, cached: true, text: subText, download_url: null };
          const song = buildAssSong(file);
          if (song) {
            await playSong(song);
            renderAssCacheList();
            ...
            // Không có YouTube ID hợp lệ → chỉ lưu cache, không phát được
            toast('Đã lưu sub vào cache. Không phát được vì thiếu YouTube ID.', 'warning', 3000);
          }
          return;
        }

        // Trường hợp 2: chọn file từ search dropdown
    ...
    $('#animeModal').addEventListener('click', (e) => {
      ...
      const delBtn = e.target.closest('.watch-date-del');
      if (delBtn && delBtn.dataset.watchDel) {
