...
- **Chi tiết anime (modal):** synopsis, studio, năm, rating, thanh tiến độ tập, dàn seiyuu (voice actors + ảnh nhân vật), liên kết 2 loại (🔗 liên kết chính + 🎬 link khác/streaming, admin sửa trực tiếp trong form) lưu sẵn trong Supabase — khách xem tất cả trong một danh sách, chỉ live-fetch AniList/Jikan khi record chưa có
- **Auto-fill từ AniList / Jikan:** nhập tên anime → tự điền đầy đủ metadata + seiyuu + liên kết (gồm link khác); fallback Jikan/MAL khi AniList tạm ngừng
